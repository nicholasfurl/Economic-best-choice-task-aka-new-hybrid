...
def analyzeSecretaryNick_python(priorProb, list):
    ...
    x = (priorProb.mu - sdevs * sqrt(priorProb.sig) + dx, dx, priorProb.mu + sdevs * sqrt(priorProb.sig) + dx)
...
def rnkBackWardInduction(sampleSeries, ts, priorProb, listLength, x, Cs, payoff_scheme):
    ...
    ties = 0
    if len(np.unique(sampleSeries[:ts])) < ts:
        ties = 1
...
####Needed to input next bit separately ... starting at for loop ti

Nx = len(utility)

for ti in range(N, ts-1, -1):
    if ti == N:
        utCont = -float("inf") * np.ones((Nx, 1))
    elif ti == ts:
        utCont = np.ones((Nx, 1)) * sum(px * utility[:, ti+1])
    else:
        utCont = computeContinue(utility[:, ti+1], postProb, x, ti)
        
    utStop = np.empty((Nx, 1)) * np.nan
    ...
    for rk in range(maxPayRank):
        pf = np.prod(list(range(td, td-rk-1, -1))) / np.math.factorial(rk)
        ps[:, rk+1] = pf * (Fpx ** (td - rk)) * ((cFpx ** rk))
        
    for ri in range(1, maxPayRank + 1):
        z = np.where((x < rnkv[ri]) & (x >= rnkv[ri+1]))[0]
